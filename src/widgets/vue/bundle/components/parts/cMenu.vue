  ...
  <div class="c-menu-wrapper">
    ...
    >
      <div class="c-menu__selection" @click.stop="$emit('toggle')">
        {{ title }}:&nbsp;<span
          v-if="(!open && selection) || showLikes || removeDislikes"
        >
          <template v-if="selectedOptions">{{ outputSelectedOptions }}</template
          ><template v-if="showLikes">Show Things I Like</template
          ><template v-if="removeDislikes">Remove Things I Dislike</template
          ><template v-if="!selectedOptions && !showLikes && !removeDislikes">{{
            selection
          }}</template></span
        >
