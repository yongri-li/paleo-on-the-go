<template>
  <div :class="_buildModifiers('c-sidebarProducts', modifiers)" v-if="content && products">
    <div class="c-sidebarProducts__list">
      <div
        class="c-sidebarProducts__item"
        v-for="(product, index) in products"
        :key="`${product.id}-${index}`"
      >
        <c-img class="c-sidebarProducts__itemImage" :src="product.images[0]" :widths="[50, 150, 200]" />
        <div
          class="c-shipmentUpsell__itemReviews yotpo bottomLine yotpo-small"
          :data-appkey="yotpo.appKey"
          data-domain="https://atlasbars-dev.myshopify.com/"
          :data-product-id="product.id"
        />
        <c-h
          class="c-sidebarProducts__itemTitle"
          tag="h4"
          level="4"
          :modifiers="['isBolder']"
          :text="product.title"
        />
        <div
          class="c-sidebarProducts__itemPrice"
          v-html="`${currencySymbol}${_formatMoney({ amount: product.defaultPrice })}`"
        />
        <c-button
          class="c-sidebarProducts__itemButton u-fullWidth"
          ...
          @click="
            UI_SET_SIDEBAR({
              ...
              content: { ...sidebarContent.add, add_back: sidebarContent.add.add_back_products },
              settings: {
